<template>
    <div>
        <div v-for="project of dataProjects" :key="project.id">
            <div v-if="$route.params.slug == project.slug" class="content-work" v-hammer:pan.vertical="slide" ref="contentWork">
                ...
                <div class="fix-title">
                    <div class="content-overflow">
                        <h1 :style="{ color: project.color }" :class="showRound ? 'no-anim' : ''">{{project.title}}</h1>
                    </div>
                </div>
                <div class="content">
                    <div class="col-8">
                    ...
                    <div class="col-4">
                        ...
                        <ul>
                            <li class="description" v-for="task in project.task">{{ task.message }}</li>
                        </ul>
                    </div>
                </div>
                <div class="list-img">
                    <div class="content-img">
                        <img :src="require(`@/assets/projects/${project.slug}/1.png`)" ref="responsive1" class="img-responsive img-1" />
                    </div>
                    <div class="content-img">
                        <img :src="require(`@/assets/projects/${project.slug}/2.png`)" ref="responsive2" class="img-responsive img-2" />
                    </div>
                    <div class="content-img">
                        <img :src="require(`@/assets/projects/${project.slug}/3.png`)" ref="responsive3" class="img-responsive img-3" />
                    </div>
                    <div class="content-img">
                        <img :src="require(`@/assets/projects/${project.slug}/4.png`)" ref="responsive4" class="img-responsive img-4" />
                    </div>
                    <div class="content-img">
                        <img :src="require(`@/assets/projects/${project.slug}/5.png`)" ref="responsive5" class="img-responsive img-5" />
                    </div>
                </div>
                <div v-show="project.helpby !== undefined" class="container-text">
                    <div class="title">Thanks</div>
                    <ul>
                        <li class="description" v-for="user in project.helpby">{{ user.author }} - {{ user.role }}</li>
                    </ul>
                </div>
                ...
                <div class="content-next-project">
                    <nextWithCanvas :successAnim="nextProject" :updateColor="updateColor" :dataProjects="dataProjects" />
                </div>
            </div>
        </div>

        <div class="circle-transition" v-for="project of dataProjects" :key="project.slug">
                    <div v-show="$route.params.slug === project.slug" class="circle-right" 
                    :style="{ backgroundColor: project.bg }" 
                    :class="slideChangeRouteActive ? 'active' : ''"></div>
                <div v-show="$route.params.slug === project.slug" class="circle-left" 
                    :style="{ backgroundColor: project.bg }" 
                    :class="slideChangeRouteActiveReverse ? 'active' : ''"></div>
        </div>
        ...
        <div class="content-link-project">
    ...
    export default {
        ...
        methods: {
            ...
            slide: function(_event) {
                ...
                if (bounding1.top - window.innerHeight + 200 <= 0 && this.imgs[0] == false) {
                    TweenLite.to('.img-1', 1.6, {
                        scale: 1,
                        ease: Circ.easeOut,
                    })
                ...
                if (bounding2.top - window.innerHeight + 200 <= 0 && this.imgs[1] == false) {
                    TweenLite.to('.img-2', 1.6, {
                        scale: 1,
                        ease: Circ.easeOut,
                    })
                    this.imgs[1] = true
                }
                if (bounding3.top - window.innerHeight + 200 <= 0 && this.imgs[2] == false) {
                    TweenLite.to('.img-3', 1.6, {
                        scale: 1,
                        ease: Circ.easeOut,
                    })
                    this.imgs[2] = true
                }
                if (bounding4.top - window.innerHeight + 200 <= 0 && this.imgs[3] == false) {
                    TweenLite.to('.img-4', 1.6, {
                        scale: 1,
                        ease: Circ.easeOut,
                    })
                    this.imgs[3] = true
                }
                if (bounding5.top - window.innerHeight + 200 <= 0 && this.imgs[4] == false) {

                    TweenLite.to('.img-5', 1.6, {
                        scale: 1,
                        ease: Circ.easeOut,
                    })
                    this.imgs[4] = true
                ...
                if (normalized.pixelY > 0) {
                    if (this.scroll + normalized.pixelY > this.$refs.contentWork[0].offsetHeight - window.innerHeight) {
                        this.scroll = this.$refs.contentWork[0].offsetHeight - window.innerHeight
                        TweenLite.to('.content-work', 0.5, {
                            y: -this.scroll
                        })
                        ...
                        TweenLite.to('.content-work', .5, {
                            y: -this.scroll
                        })
                        TweenLite.to('.content-next-project', .5, {
                            y: this.scroll
                        })
                    }
                } else {
...
h1 
    ...
    margin:0
    transition: .5s ease-in-out;
    animation: showTitle .4s .5s ease-in-out forwards
...
.no-anim 
    transform:translateY(0%)
    animation:hideTitle .5s 0.7s ease-in-out forwards;
